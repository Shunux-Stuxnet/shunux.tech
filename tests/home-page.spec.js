...
test.describe('page: index', () => {
	...
	test('shows a working cta button for the skills', async ({ page }) => {
		const skillsCta = await page.locator('aside .marker-link')
		await expect(skillsCta).toBeDefined

		await skillsCta.click()
	...
	test('shows my latest posts', async ({ page }) => {
		...
		await expect(firstEntry).toHaveAttribute('href', /skills/)

		const secondEntry = cvEntries.last()
		await expect(secondEntry).not.toHaveAttribute('target', '_blank')
		await expect(secondEntry).toHaveAttribute('href', /skills/)
	})
})
